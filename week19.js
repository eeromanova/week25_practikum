...
function makeOne() {
  fetch("https://catfact.ninja/fact")
    ...
    .catch((err) => {
      console.log("Не удается выполнить запрос: ", err);
    });
  //Ваш код
}
...
function makeTwo() {
  const paragraph2 = document.getElementById("result2");
...
function makeSix() {
  fetch("https://www.boredapi.com/api/activity")
    ...
    .then((data) => {
      console.log(
        `Активность: ${data.activity}, тип: ${data.type}, цена: ${data.price}, доступность: ${data.accessibility}`
      );
    })
    .catch((err) => {
      console.log("Не удается выполнить запрос: ", err);
...
function makeTwelve() {
  ...
  fetch("http://ip-api.com/json/" + ipAddress)
    ...
    .then((data) => {
      ...
      resultElement.textContent = data.country;
      //Ваш код
    })
    .catch((error) => {
      console.error("Ошибка:", error);
